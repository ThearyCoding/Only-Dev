...
class _BetterPlayerMaterialControlsState
  ...
  late Timer _hideSeekEffectTimer = Timer(Duration.zero, () {});
        ...
        return GestureDetector(
          ...
          child: AbsorbPointer(
            ...
            child: Stack(
              fit: StackFit.expand,
              children: [
                _buildSeekEffect(left: true),
                _buildSeekEffect(left: false),
                if (_wasLoading)
                ...
                Positioned(
                  ...
                  left: 0,
                  right: 0,
                  child: _buildFastForwardAnimation(),
                ),
                Positioned(
                  top: 0,
                  left: 0,
  ...
  void _showSeekEffect(bool left) {
    setState(() {
      if (left) {
        _showLeftSeek = true;
      } else {
    ...
    _hideSeekEffectTimer.cancel();

    _hideSeekEffectTimer = Timer(Duration(milliseconds: 500), () {
      setState(() {
        _showLeftSeek = false;
  ...
  Widget _buildSeekEffect({required bool left}) {
    final double horizontalMargin = _isFullscreenMode() ? 50.0 : 100.0;

    return Align(
      alignment: left ? Alignment.centerLeft : Alignment.centerRight,
      child: AnimatedOpacity(
        opacity: left ? (_showLeftSeek ? 1 : 0) : (_showRightSeek ? 1 : 0),
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  _buildAnimatedIcon(left, 1),
                  _buildAnimatedIcon(left, 2),
                ],
              ),
              AnimatedOpacity(
                opacity: left
                    ? (_showLeftSeek ? 1.0 : 0.0)
                    : (_showRightSeek ? 1.0 : 0.0),
                duration: const Duration(milliseconds: 300),
                child: Text(
                  left ? '-10s' : '+10s',
                  style: TextStyle(
  ...
  Widget _buildAnimatedIcon(bool left, int index) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        AnimatedScale(
          scale:
              left ? (_showLeftSeek ? 1.5 : 1.0) : (_showRightSeek ? 1.5 : 1.0),
  ...
  Widget _buildFastForwardAnimation() {
    return AnimatedOpacity(
      opacity: isSpeedIncreased ? 1.0 : 0.0,
  ...
  Widget _buildPosition() {
    ...
    return Padding(
      padding: _controlsConfiguration.enablePlayPause
          ? const EdgeInsets.only(right: 24, left: 10)
